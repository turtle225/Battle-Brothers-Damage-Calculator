#Battle Brothers Damage Calculator -- 1Hander Battery Version 1.7.0:
...
#1Hander specific:
DoubleGrip = 1          #Note: DoubleGrip will automatically turn off when Throwing tests run. 
Duelist = 1             #Note: Set DoubleGrip if using Duelist.
#Perks:                 
CripplingStrikes = 0
Executioner = 0
...
#Attacker Stats: #Begins with Winged Mace.
Mind = 35
Maxd = 55
Headchance = 25
Ignore = 40
...
#-- Added a Hammer test with Destroy Armor to the test batch.
...
#-- Adjusted 3Head and Throwing trials, and added a Qatal Deathblow trial.
...
#-- Fixed Throwing Axe ignore% to 30 (from 25) in the test. Tooltip lists 25% ignore, but the "Throw Axe" skill modifies it to 30%.
...
#-- Fixed a mistake exclusive to this calculator variant where the ShamshirMastery tag was not properly turning off after the Shamshir test, causing the injury bonus effect to persist on the dagger, spear, and throwing tests that ran after.
